...
| **`var` (Type Inference)** | 286 | Java 10 | Local variable only: `var map = new HashMap<String, List<Trade>>()` | `module1/var` |
...
| **Pattern Matching for `instanceof`** | 394 | Java 16 | Test + bind in one step: `if (e instanceof TradeCreated c)` — no redundant cast | `module2/patternmatching` |
| **Sealed Classes** | 409 | Java 17 | `sealed interface TradeEvent permits TradeCreated, TradeRejected` — closed-world hierarchy | `module2/sealedclasses` |
...
| **Record Patterns** | 440 | Java 21 | Deconstruct a record in `instanceof`: `if (obj instanceof Trade(String id, _, Money(double amt, _)))` | `bonus/recordpatterns` |
| **Pattern Matching in Switch** | 441 | Java 21 | `switch(event) { case TradeCreated c -> … }` — type patterns as case labels, `when` guards | `bonus/patternswitch` |
...
// Group by status
Map<String, List<Trade>> byStatus = trades.stream()
...
public record TradeDetailsRecord(String tradeId, String symbol, double notional) {
    public TradeDetailsRecord {
        ...
        if (notional < 0) throw new IllegalArgumentException("notional >= 0");
    }
...
```java
String sql = """
        SELECT t.id, t.symbol, t.notional
        FROM   trades t
        WHERE  t.status = '%s'
...
### Sealed + Records
```java
sealed interface TradeEvent permits TradeCreated, TradeRejected {
    String tradeId();
}
record TradeCreated(String tradeId, String symbol, double notional) implements TradeEvent {}
record TradeRejected(String tradeId, String reason)                 implements TradeEvent {}
```

### Pattern Matching + Switch (Java 17)
```java
String result = switch (event) {
    case TradeCreated c  -> "Created: "  + c.tradeId();
    case TradeRejected r -> "Rejected: " + r.reason();
};
```
...
try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
    var cpTask    = scope.fork(() -> fetchCounterparty(id));
    var priceTask = scope.fork(() -> fetchPrice(symbol));
    scope.join().throwIfFailed();
    return new EnrichedTrade(cpTask.get(), priceTask.get());
}
```
...
|-------------|----------------|
| `trade.getTradeId()` | `trade.tradeId()` |
| `trade.getSymbol()` | `trade.symbol()` |
| `trade.getNotional()` | `trade.notional()` |
